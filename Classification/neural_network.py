...
for guilty in [False, True]:
    train, valid, test = PreProcessing(cases_path, guilty)
    dc_i = 0
    for dc in dataset_classes:
        train_dataloader = DataLoader(dc(train), batch_size=128).to(DEVICE)
        valid_dataloader = DataLoader(dc(valid), batch_size=128).to(DEVICE)
        ...
        test_dataloader = DataLoader(DataDispFem(test), batch_size=128).to(DEVICE)
